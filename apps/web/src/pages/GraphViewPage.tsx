...
function toTargetKey(type: string, value: unknown): string | null {
  ...
  if (value && typeof value === "object") {
    ...
    if (typeof r.type === "string" && typeof r.id === "string")
      return nodeKey(r.type, r.id);
    if (
      typeof r.resource_type === "string" &&
      typeof r.resource_id === "string"
    )
      return nodeKey(r.resource_type, r.resource_id);
    if (typeof r.type === "string" && typeof r.resource_id === "string")
      return nodeKey(r.type, r.resource_id);
  }
  return null;
}
...
function buildGraph(
  ...
  for (const [type, resources] of Object.entries(resourcesByType)) {
    ...
    for (const resource of resources) {
      ...
      for (const [targetType, raw] of Object.entries(
...
export function GraphViewPage() {
  ...
  return (
    <div className="space-y-4">
      <PageHeader
        title="Graph View"
        subtitle="Resource relations across references and dependencies."
      />
      ...
      <div className="grid gap-4 lg:grid-cols-[1fr_320px]">
        <div className="rounded-2xl border border-gray-200 bg-white p-3">
          ...
          <div
            ref={containerRef}
            className="h-[65vh] min-h-[420px] w-full overflow-hidden rounded-xl bg-slate-950/95"
          >
            <svg
              width={size.width}
              height={size.height}
        ...
        <aside className="rounded-2xl border border-gray-200 bg-white p-4">
          ...
          {!selectedNode ? (
            <p className="mt-3 text-sm text-gray-600">
              Select a node to inspect its relations.
            </p>
          ) : (
            <div className="mt-3 space-y-3">
              <div>
                ...
                <p className="mt-1 text-xs text-gray-600">
                  Connected edges: {degree.get(selectedNode.id) ?? 0}
                </p>
                <ActionLink
                  to={`/resources/${selectedNode.type}/${selectedNode.resourceId}`}
                  tone="primary"
              ...
              <div>
                ...
                <ul className="mt-2 max-h-[45vh] space-y-2 overflow-auto pr-1">
                  {selectedEdges.length === 0 ? (
                    <li className="text-sm text-gray-600">No relations</li>
                  ) : null}
                  {selectedEdges.map((edge, idx) => {
                    ...
                    return (
                      <li
                        key={`${otherId}-${idx}`}
                        className="rounded-lg border border-gray-200 bg-gray-50 px-2 py-1.5 text-xs text-gray-700"
                      >
