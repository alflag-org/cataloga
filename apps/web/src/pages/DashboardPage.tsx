...
export function DashboardPage() {
  ...
  const counts = useMemo(() => Object.fromEntries(Object.entries(resourceByType).map(([k, v]) => [k, v.length])), [resourceByType])
  ...
  const searchHits = useMemo(() => {
    ...
    for (const t of types) {
      ...
      for (const r of rows) {
        const haystack = [r.metadata.id, r.metadata.name, r.metadata.type, JSON.stringify(r.metadata.tags), JSON.stringify(r.spec)].join(' ').toLowerCase()
        if (haystack.includes(q)) {
          hits.push({ type: t.id, typeTitle: t.title || t.id, id: r.metadata.id, name: r.metadata.name || r.metadata.id })
        }
      }
    }
  ...
  return (
    <section className="space-y-5">
        ...
        <div className="space-y-5">
          {Object.entries(grouped).map(([group, groupTypes]) => (
            <div key={group} className="space-y-3">
              <h3 className="text-sm font-semibold uppercase tracking-wide text-gray-700">{group}</h3>
      ...
      <DataCard title="Basic validation">
        <p className="text-sm text-gray-700">{validationStatus === 'failed' ? 'Failed' : validationStatus === 'ok' ? 'OK' : 'Unknown'}</p>
        {validationStatus === 'failed' || validationWarnings > 0 || validationErrors > 0 ? (
          <div className="pt-2 text-sm text-gray-700">
            <p>Errors: {validationErrors}</p>
            <p>Warnings: {validationWarnings}</p>
            <Link to="/validation" className="mt-2 inline-block text-blue-700 hover:text-blue-800">
